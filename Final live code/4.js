...
function train(pokemon, candy) {
  ...
  var totalLevel = pokemon.level + candy
  if(pokemon.evolutionLine === undefined){
    return 'Congratulations, your ' + pokemon.name +' grew to LV. ' + totalLevel + '!'
  }
  for(var i=0;i<pokemon.evolutionLine.length;i++){
      return 'Congratulations, your ' + pokemon.name +' grew to LV. ' + totalLevel + '! And it evolved into '+ pokemon.evolutionLine[i].name +' and '+ pokemon.evolutionLine[i+1].name +'!'
  }
}
